...
function ProgressBar({ current, total, width = 16 }: { current: number; total: number; width?: number }) {
  if (total === 0) {return <span>{EMPTY.repeat(width)}</span>;}
  ...
  return (
    <box border borderColor={colors.progressBorder} padding={1} marginBottom={1} flexDirection="column">
      ...
      {progressItems.map((item, _idx) => {
        ...
        const fg = item.status === "done" ? colors.statusDone
          : item.status === "error" ? colors.statusError
          : item.status === "processing" ? colors.statusProgress
          : colors.dim;
        return (
          <text key={item.folderPath} fg={fg}>
            {icon} {item.folderName}{" "}
            <ProgressBar current={item.pagesCompleted} total={item.pagesTotal} />
            {" "}{item.pagesCompleted}/{item.pagesTotal} pages{item.message && item.status === "error" ? ` \u2014 ${item.message}` : ""}
          </text>
        );
      })}
      <text fg={colors.dim}>{"\u2500".repeat(40)}</text>
      <text fg={colors.statusInfo}>
        {t("progress.overall", { pages: totalPages, elapsed })}
      </text>
    </box>
  );
}
